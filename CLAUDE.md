...
- **Malformed tool-call retry**: models that emit XML-dialect tool calls (`<function=x><parameter=y>`, e.g. Qwen) sometimes drop a closing tag, and Ollama rejects the whole response with a Go parser error (`XML syntax error … element <parameter> closed by </function>`). This is a sampling slip, not a real failure, so `agent_loop` re-samples the step up to `MALFORMED_TOOL_CALL_RETRIES` (4) times before surfacing the error — otherwise one bad sample kills a run that had already done its work. Measured: `qwen3.6` fails this way in ~38% of samples once the tool list is large (39 tools), while qwen3.8 / glm-4.7-flash / gemma4 never did — so it is a per-model Ollama template fault, and switching model is the real fix
...
- **Oversized tool-result offload** (`ollama::offload_tool_result`): a tool result larger than the per-profile limit is truncated for the model's context, but in interactive chat the **full** result is also written to a file under a temp dir (`lexichat-tool-results/`), and the truncation marker tells the model the path and to use `run_python` to process it. `run_python` gets read access via `dispatch_paths` (sandbox_paths + that dir). This lets the model count/aggregate large API responses (e.g. hundreds of police-crime records) exactly instead of guessing from a truncated slice. Background jobs can't run code, so they get plain truncation (`cap_tool_result`).
- **Per-run read cache** (`agent_loop`, `cache_effect`/`read_cache_key`): successful (`HTTP 2xx`) OpenAPI GET and SPARQL query results are cached for the rest of the run, keyed by tool + key-sorted args; a repeat call is served from it with a "reused, not re-fetched" note appended (after cap/offload, so offload files stay raw JSON). Any OpenAPI write clears it. Not shared across turns, and not used by code-mode `call_tool` (`call_one_tool`). The loop breaker still stops a third identical call regardless
- **Per-step tool selection** (`ollama::select_tools_for_step`): tools are grouped (built-ins, and one group per OpenAPI spec / SPARQL endpoint / MCP server) and, when the discoverable total exceeds `SELECTION_THRESHOLD` (25), narrowed **per step** to those relevant to the current step. Two-level: an LLM pre-flight picks relevant groups, then (only if still over the cap) picks specific operations. Selection runs inside `agent_loop` each step using a context built from the task + recent activity, so a multi-API chain gets the right tools as it progresses; tools called last step are kept for continuity. The profile's `maxTools` (sent as `max_tools`, default `DEFAULT_TOOL_CAP` = 40) is the hard cap. `always_tools` (the wiki workflow) bypass selection. This is what makes "enable many OpenAPI specs" viable — hundreds of tools never all reach the model at once.
